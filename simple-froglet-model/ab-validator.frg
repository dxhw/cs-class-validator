...
lone sig oldDegreeAB extends Degree {
    calc: one Boolean,
    intro1: one Course,
    intro2: one Course,
...
pred finished_calc_oAB {
    all d: oldDegreeAB | {
        some d.calc
    }
}

pred valid_intro_oAB {
    all d: oldDegreeAB | {
        ...
        not reachable[d.intro1, d, inter1, inter2, inter3, elec1, upperLevel,
                pathway1, core1, coreOrRelated, intermediate1, intermediate2, intermediate3]
        not reachable[d.intro2, d, inter1, inter2, inter3, elec1, upperLevel,
                pathway1, core1, coreOrRelated, intermediate1, intermediate2, intermediate3]
    }
}

pred valid_intermediate_oAB {
    //For all degrees...
    all d: oldDegreeAB | {
        ...
        some disj c1, c2, c3: Course | {       
            ...
            ((c1.intermediateType = FoundationsI and c2.intermediateType = MathematicsI) or
            (c1.intermediateType = FoundationsI and c2.intermediateType = SystemsI) or
            (c1.intermediateType = MathematicsI and c2.intermediateType = SystemsI))
        }  
    } 
}
...
pred valid_upper_level_oAB {
    all d: oldDegreeAB | {
        ...
        not reachable[d.upperLevel, d, intro1, intro2, inter1, inter2, inter3, elec1, 
                pathway1, core1, coreOrRelated, intermediate1, intermediate2, intermediate3]
    }
}

pred valid_electives_oAB {
    all d: oldDegreeAB | some disj e1 : Course |  {
        ...
        all c1: Course |  (
            ...
            reachable[c1, d, intro1, intro2, inter1, inter2, inter3, upperLevel, 
                pathway1, core1, coreOrRelated, intermediate1, intermediate2, intermediate3]
        ) implies {
            e1 != c1
        }
...
pred wellformed_degree_AB {
    finished_calc_oAB
    wellformed_course
    valid_intro_oAB
    valid_intermediate_oAB
    all_pathways_valid
    valid_upper_level_oAB
    valid_electives_oAB
    ...
    (#{c: Course | some c.artsy} >= 0) and (#{c: Course | some c.artsy} <= 2)
...
run {
    wellformed_degree_AB
} for exactly 1 oldDegreeAB, exactly 9 Course